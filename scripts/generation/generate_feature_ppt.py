...
# --- IMAGE PATHS (Update these with the results from find_by_name) ---
# Use local relative path which we just populated
...
def add_feature_slide(prs, title, points, img_key=None, accent_color=ACCENT_1):
    ...
    if img_path and os.path.exists(img_path):
        ...
        # Add a nice border?
        # Decoration only available on shape objects usually, simple picture is fine.
...
if __name__ == "__main__":
    try:
        ...
        with open("ppt_error_log.txt", "w") as f:
            f.write(str(e))
            import traceback
