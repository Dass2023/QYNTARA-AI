    ...
    def toggle_sync(self, checked):
        ...
        try:
            ...
            if checked:
                ...
                pipeline = "Standard"
                target_engine = "Digital Twin Core"
                
                # Try to access Main Window combo
                parent = self.window()
                if hasattr(parent, 'combo_pipeline'):
                    pipeline = parent.combo_pipeline.currentText()
                    
                if "Game" in pipeline: target_engine = "Unreal Engine 5"
                elif "Web" in pipeline: target_engine = "Three.js Client"
                elif "VR" in pipeline: target_engine = "Unity XR"
                
                success = core.connect_live_stream(asset_id)
                if success:
                    self.btn_sync.setText(f"LINKED: {target_engine.upper()}")
                    self.btn_sync.setStyleSheet("background: #ff9900; color: #000; font-weight: bold;")
                    self.lbl_telemetry.setText(f"Telemetry: STREAMING TO {target_engine.upper()} (120Hz)...")
