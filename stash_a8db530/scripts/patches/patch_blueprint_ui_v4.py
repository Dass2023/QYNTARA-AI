...
new_method = """    def run_ai_analysis(self):
        ...
        try:
            ...
            reply = QtWidgets.QMessageBox.question(self, "Deep AI Analysis", 
                                                 ...
                                                 f"(Floors, Ceilings, Labels, & Textures)",
                                                 QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
                                                 
            if reply == QtWidgets.QMessageBox.Yes:
                QtWidgets.QApplication.setOverrideCursor(QtGui.QCursor(QtCore.Qt.WaitCursor))
                # Pass wall_height from spinner AND Image Path
                ...
                if created:
                    ...
                    cmds.select(created)
                    QtWidgets.QMessageBox.information(self, "Success", f"Generated {len(created)} 3D Elements with Blueprint Textures.")
...
if start_idx != -1:
    ...
    with open(target_file, "w") as f:
        f.write(new_content)
    print("Success: Patched run_ai_analysis w/ Texture.")
else:
