...
def run_debug_env():
    ...
    try:
        ...
        if hist_fail:
            ...
            # Verify
            hist = cmds.listHistory("BadCube", pdo=True)
            if not hist: # Should only have shape? listHistory behaves differently. 
                # Better check: delete history leaves no construction history nodes
                # listHistory often returns the shape itself.
                # Let's check if 'polyExtrude' is gone.
