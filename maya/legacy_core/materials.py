...
def check_default_material(objects):
    ...
    for obj in objects:
        ...
        for shape in shapes:
            ...
            for sg in sgs:
                # Direct shader query is safer than broad listConnections + ls
...
def check_multi_materials(objects):
    ...
    for obj in objects:
        ...
        for shape in shapes:
            ...
            # Filter specifically for the Shading Engine nodes
            unique_sgs = [sg for sg in unique_sgs if cmds.nodeType(sg) == 'shadingEngine']
            
            if len(unique_sgs) > 1:
                violations.append({
                    "object": obj,
                    "issue": f"Multiple Materials ({len(unique_sgs)})",
                    "components": unique_sgs,
        ...
        for shape in shapes:
             ...
             for sg in sgs:
                 ...
                 for m in members:
                     ...
                     if m_base == short_obj or m_base == short_shape:
                         if '.f[' not in m:
                             fully_assigned = True
                             break
                         else:
                             faces = cmds.ls(m, flatten=True)
             ...
             if len(assigned_faces) < num_faces:
                 all_faces = set(cmds.ls(f"{obj}.f[*]", flatten=True))
                 missing_faces = list(all_faces - assigned_faces)
                 
                 violations.append({
                     "object": obj,
                     "issue": "Missing Material (Partial/Per-Face)",
                     "components": missing_faces,
                     "action": "Assign Material",
...
def check_texture_size(objects, max_res=2048):
    ...
    for file_node in file_nodes:
        ...
        except Exception as e: raise RuntimeError(f"Validation Crash: {e}") # Failed to read res
